	...
	<v-card :dark="this.$store.getters.hasdarkflag">
		...
		>
			...
			<template v-slot:[`item.status`]="{ item }">
				<v-chip
					:color="getColor(item.status, $store.getters.hasdarkflag)"
					:dark="$store.getters.hasdarkflag"
				>
					{{ item.status }}
				</v-chip>
...
export default {
	...
	methods: {
		...
		getColor(status, dark) {
			if (status === "Por entregar")
				return dark ? "orange darken-1" : "orange lighten-1";
			else if (status === "Entregado")
				return dark ? "blue darken-1" : "blue lighten-1";
			else if (status === "Cerrado")
				return dark ? "red darken-2" : "red	lighten-2";
			else if (status === "Cancelado")
				return dark ? "grey darken-1" : "grey lighten-1";
			else return dark ? "deep-orange darken-4" : "deep-orange lighten-4";
		},
		close() {
			this.dialog = false;
			this.$nextTick(() => {
