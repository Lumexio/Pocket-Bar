...
export default {
  ...
  methods: {
	...
	connectToSocket(channel, event, variableName, callbackVariableName) {
        ...
        event,
        (e) => {
          this[variableName] = this.parseNotifications(
            e[callbackVariableName]
          );
  ...
  computed: {
    hasrol() {
      ...
      if (this.$store.getters.hasrol === 4) {
        rol = "mesero";
		this.connectToSocket("mesero.", "MeseroEvents", "ticketsPWANotiArrayMesero", "TicketsARecibir");
      } else if (this.$store.getters.hasrol === 5) {
        rol = "bartender";
		this.connectToSocket("barra.", "BarraEvents", "ticketsPWANotiArrayBarra", "notificacionesBarra");
      }
